...
export type ReportRow = Report & { analysis_id: string; brand?: string | null; variant?: string | null };

/* Shared report renderer (PRD §8.3/§8.6.6): used in-flow on the scan progress page
   (below the pinned stepper) and on /report/[id]. NOTE: placeholder report layout;
   final format/copy is D's (report_template.md). Read flow: content in solid cards. */
export function ReportView({ report, name }: { report: ReportRow; name?: string | null }) {
  ...
  async function sendFeedback() {
    ...
    await api.feedback(report.analysis_id, thumb, comment || undefined).catch(() => null);
    setFeedbackSent(true);
  }

  return (
    <>
      {name && <p className="text-sm text-ink-muted">Hi {name}, here&rsquo;s our read:</p>}
      ...
      {/* report feedback: test-flow instrumentation (PRD §8.6.6); removed for the main flow later */}
          ...
          <>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <input
                ...
                className="min-h-[44px] min-w-[160px] flex-1 rounded-md border border-hairline-strong bg-paper px-3 text-sm text-ink placeholder:text-ink-faint"
              />
