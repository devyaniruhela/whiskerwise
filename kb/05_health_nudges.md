...
|---|---|---|---|---|
| `obesity_weight` | 6–9 ("a bit chonky" / "overweight") | "{cat_name}'s body-condition check flags them as on the heavier side. If your vet agrees weight loss is appropriate, the standard approach is to reduce calorie intake **gradually over several weeks** — sudden calorie cuts can be harmful. Dry food is calorie-dense for its volume, so free-feeding kibble makes it easy to overshoot calories without noticing; cutting back on dry food (or shifting some intake to wet food) often matters more than any single ingredient choice. Obesity raises the risk of diabetes, joint strain, and other complications, so it's worth taking seriously — talk to your vet about a weight-management plan for {cat_name}. This analysis doesn't calculate {cat_name}'s calorie needs or portion size." | Cornell Care of Obese Cats; FEDIAF "Maintaining a healthy weight for your cat" | `added` |
...
- Whether to source `underweight_weight` properly (no corpus doc currently covers underweight cats specifically — the Cornell/FEDIAF docs in `therapeutic/` are obesity-focused) before shipping its specific calorie-density claim.
